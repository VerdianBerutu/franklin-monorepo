<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-screen overflow-y-auto p-8">
      ...
      <form v-if="customer" @submit.prevent="submit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Nama Lengkap *</label>
            <input v-model="form.name" required type="text" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Perusahaan</label>
            <input v-model="form.company" type="text" class="w-full px-4 py-2 border rounded-lg">
          </div>
          <div>
          ...
          <div>
            ...
            <input v-model="form.phone" type="text" class="w-full px-4 py-2 border rounded-lg">
          </div>
          <div>
        ...
        <div class="flex gap-4 mt-8">
          ...
          <button type="submit" :disabled="saving" class="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-70">
            {{ saving ? 'Menyimpan...' : 'Update Customer' }}
          </button>
        </div>
      </form>
    </div>
...
const props = defineProps(['isOpen', 'customer'])
